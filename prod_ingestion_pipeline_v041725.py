...
Analyze the provided text content from a single page ({page_num}) of a document. Your goal is to segment this text into logical subsections.
...
3.  For each subsection, determine:
    ...
    *   `description`: A brief, one-sentence summary of this subsection's main point.
    *   `is_cutoff`: Set to `true` ONLY if this specific text chunk appears to end mid-sentence or mid-thought, suggesting it was truncated *before* being passed to you (this is less likely here but possible if original text was huge). Otherwise `false`.
    *   `referenced_visuals`: An empty list `[]`. (Visuals were extracted previously).
    *   `referenced_tables`: An empty list `[]`. (Tables were extracted previously).

4.  Return ONLY a valid JSON array containing the subsection objects, structured exactly like this:
    ```json
    [
      {{
        ...
        "description": "This section introduces the main topic.",
        "is_cutoff": false,
        "referenced_visuals": [],
        "referenced_tables": []
      }},
    ...
    for attempt in range(max_structure_retries):
        ...
        try:
            response = await retry_api_call( # This handles API-level retries
                client.aio.models.generate_content,
                model="gemini-2.0-flash", # Use a recent flash model - good balance
...
async def extract_structure_from_text_content(client: genai.Client, text_content: str, page_num: int, filename: str) -> PageContent:
    ...
    try:
        ...
        response = await retry_api_call(
            client.aio.models.generate_content,
            model="gemini-2.0-flash", # Flash should be fine for this structure extraction
...
async def analyze_concept_entity_relationships(client: genai.Client, chapter: Chapter, max_structure_retries: int = 2, retry_delay: int = 3) -> None:
    """
    ...
    """
    function_name = "analyze_concept_entity_relationships_refactored" # Renamed slightly
    chapter_id = getattr(chapter, 'chapter_id', 'unknown_chapter')
    chapter_title = getattr(chapter, 'title', 'Untitled Chapter')
    logging.info(f"[{function_name}] Analyzing relationships for Chapter '{chapter_id}': '{chapter_title}'")
...
1c. **Identify Relationships (Linked Nodes):** For each node created (the *source* node), find its most important (max 5-7) direct relationships *to other nodes also created in this Task* (the *target* nodes). Focus on explicit or clearly implied connections within the text.
    *   For each relationship found, create a `linked_nodes` object containing:
        ...
        *   `relationship_description`: A brief explanation of how the source node relates to the target node (e.g., "uses technique", "developed by", "is a component of", "competes with").
        *   `relationship_keywords`: (Optional) 1-3 keywords for the relationship type (e.g., ["dependency", "causation"]). Default to `[]` if none apply.
        *   `relationship_strength`: Estimate the importance/strength of this specific link (float from 1.0 to 10.0).
...
*   **SCHEMA ADHERENCE:** Follow the provided schema precisely. All REQUIRED fields must be present with the correct data types (string, float, list). `linked_nodes` list is REQUIRED but can be empty `[]`. `relationship_keywords` is OPTIONAL (defaults to `[]` if omitted).
...
*   **TEXTUAL BASIS:** All information (`description`, relationships) must be derived SOLELY from the provided "Chapter Content". Do not add external knowledge.
...
"""
    ...
    for attempt in range(max_structure_retries):
        ...
        try:
            response = await retry_api_call(
                client.aio.models.generate_content,
                model="gemini-2.5-pro-preview-03-25", # Keep capable model
    ...
    if final_data is not None:
        ...
        if isinstance(qdrant_nodes_data, list):
            ...
            for node_dict in qdrant_nodes_data:
                 ...
                 try:
                     # Use setdefault ensuring list type for linked_nodes and keywords
                     ...
                     logging.warning(f"[{function_name}] Pydantic validation failed node in chapter '{chapter_id}': {node_error}. Skipping. Data: {node_dict}")
...
async def analyze_inter_chapter_relationships(client: genai.Client, chapters: List[Chapter], doc_name: Optional[str] = "Document") -> Optional[Dict[str, Any]]:
    """
    ...
    Uses gemini-2.5-pro-preview-03-25.
    """
    function_name = "analyze_inter_chapter_relationships" # For logging context
    logging.info(f"[{function_name}] Starting analysis for {len(chapters)} chapters of '{doc_name}'.")
    ...
    ---Goal---
    Analyze the provided list of concepts and entities (nodes) aggregated from all chapters of the document '{doc_name}'. Perform two tasks:
    ...
    *   DO NOT include the original node definitions in the output.
    ...
    try:
        logging.debug(f"[{function_name}] Sending combined analysis request for '{doc_name}' to LLM ({'gemini-2.5-pro-preview-03-25'}).")
        response = await retry_api_call(
            client.aio.models.generate_content,
            model="gemini-2.5-pro-preview-03-25", # Use the specified powerful model - **NOTE: Ensure correct model name per Gemini API docs**
            contents=[
    ...
    logging.info(f"[{function_name}] Finished analysis for '{doc_name}'. Returning summary.")
    return document_summary_output # Return the summary dict or None
...
async def generate_project_ontology(client: genai.Client, documents: List[Dict]) -> ProjectOntology:
    """
    Generate a simplified project-wide ontology primarily based on
    pre-generated document summaries.
    """
    function_name = "generate_project_ontology_simplified" # Rename for clarity
    logging.info(f"[{function_name}] Starting ontology generation using document summaries for {len(documents)} documents.")

    # --- 1. Aggregate Document Summary Data ---
    documents_summary_data = [] # List of dicts {title, themes, summary_text}
    doc_titles = []
    doc_count = 0

    for doc in documents:
        if not isinstance(doc, dict) or "raw_extracted_content" not in doc:
            logging.warning(f"[{function_name}] Skipping invalid document structure: {type(doc)}")
            continue

        raw_content = doc["raw_extracted_content"]
        doc_filename = raw_content.get("filename", f"Unknown_Doc_{doc_count}")
        summary = raw_content.get("summary") # This should be the dictionary generated earlier

        if not isinstance(summary, dict):
             logging.warning(f"[{function_name}] Skipping document '{doc_filename}' due to missing or invalid summary dictionary.")
             doc_titles.append(doc_filename + " (No Summary)") # Still count doc
             doc_count += 1
             continue

        doc_title = summary.get("title", doc_filename)
        doc_titles.append(doc_title)
        doc_count += 1

        # Collect relevant summary info for the prompt
        documents_summary_data.append({
            "title": doc_title,
            "themes": summary.get("themes", []),
            "summary_text": summary.get("summary", "No summary text available.")
        })

    if not documents_summary_data:
        logging.warning(f"[{function_name}] No valid document summaries found to generate ontology.")
        return ProjectOntology(
            title="Project Ontology (No Data)",
            overview="No valid document summaries were available for analysis.",
    ...
    logging.info(f"[{function_name}] Aggregated summaries from {len(documents_summary_data)} documents for ontology prompt.")

    # --- 2. Create New Prompt Based on Summaries ---
    ontology_prompt = f"""
    ---Goal---
    Analyze the provided summaries and themes from multiple documents within a project. Synthesize this high-level information to create a project-level ontology, including an overview, overarching themes, and key concepts (as best as can be inferred from summaries).

    ---Input Data---
    A list of document summaries:
    ```json
    {json.dumps(documents_summary_data, indent=2)}
    ```
    Total Documents Analyzed: {doc_count}

    ---Instructions---
    1.  **Synthesize Overview:** Read through all the document titles, themes, and summaries. Write a comprehensive `overview` (3-6 sentences) that summarizes the main subjects, scope, and purpose of the entire project based *only* on the provided summary information.
    2.  **Identify Global Themes:** Aggregate and consolidate the themes from individual documents to determine 3-7 high-level `global_themes` that represent the major recurring topics across the project.
    3.  **Infer Key Concepts:** Based on the summaries and themes, identify the most likely `key_concepts` (5-10 distinct concepts) central to the project. Acknowledge that this is inferred from summaries.
    4.  **Generate Project Title:** Create a concise and descriptive `title` for the overall project (max 10 words) based on the synthesized understanding.
    ...
    {{
      ...
      "overview": "Comprehensive project overview text...",
      "global_themes": ["Theme 1", "Theme 2", "Theme 3"],
      "key_concepts": ["Key Concept A (inferred)", "Key Concept B (inferred)", "Key Concept C (inferred)"]
    }}
    ```
    ...
    # --- 3. Call LLM and Process Response ---
    ontology_output_data = {}
    try:
        logging.debug(f"[{function_name}] Sending summary-based ontology generation request to LLM.")
        # Flash might be sufficient for summarizing summaries
        response = await retry_api_call(
            client.aio.models.generate_content,
            model="gemini-2.0-flash", # Can try Pro if results are poor
            contents=[
        ...
        if response.candidates:
            json_text = clean_json_response(response.candidates[0].content.parts[0].text)
            ontology_output_data = json.loads(json_text)

            if not isinstance(ontology_output_data, dict) or not all(k in ontology_output_data for k in ["title", "overview", "global_themes", "key_concepts"]):
                logging.error(f"[{function_name}] LLM response for ontology missing required keys.")
                raise ValueError("LLM response missing required ontology keys.")

            logging.info(f"[{function_name}] Successfully received ontology data from LLM based on summaries.")

        else:
            logging.error(f"[{function_name}] Ontology generation response from LLM had no candidates.")
            raise ValueError("LLM response had no candidates")

    except (json.JSONDecodeError, ValidationError, ValueError, Exception) as e:
        logging.error(f"[{function_name}] Error generating project ontology or parsing response: {e}", exc_info=True)
        # Return basic ontology on error
        return ProjectOntology(
            title="Project Ontology (Error)",
            overview=f"Error occurred during ontology generation based on summaries: {e}",
            document_count=doc_count,
            documents=doc_titles,
            global_themes=[],
            key_concepts=[]
    ...
    # --- 4. Create Final ProjectOntology Object ---
    try:
         final_ontology = ProjectOntology(
             title=ontology_output_data.get("title", f"Project Ontology for {doc_count} Docs"),
             overview=ontology_output_data.get("overview", "No overview generated."),
             document_count=doc_count,
             documents=doc_titles,
             global_themes=ontology_output_data.get("global_themes", []),
             key_concepts=ontology_output_data.get("key_concepts", [])
         )
         logging.info(f"[{function_name}] Project ontology created successfully from summaries.")
        ...
        return ProjectOntology(
            ...
            overview=f"Error creating final ontology object from summaries: {ve}",
            document_count=doc_count,
            documents=doc_titles,
            global_themes=[],
            key_concepts=[]
...
async def process_single_page_with_semaphore(semaphore, client, page_info: dict, uploaded_files):
    ...
    async with semaphore:
        ...
        try:
            ...
            logging.info(f"Successfully validated PageContent for page {page_num} of {doc_name}")

        except pydantic.ValidationError as ve:
            # This catch block is now the primary defense against malformed dictionaries
            # coming from process_single_pdf_page (which indicates a bug there or in create_error_page)
            logging.error(f"Pydantic validation error creating PageContent for page {page_num} of {doc_name} from returned dictionary: {ve}")
            # Replace st.warning with logging or keep if Streamlit context is intended
            ...
            result_page_obj = create_error_page(
                ...
                validation_errors=ve.errors() # Pass validation errors if create_error_page uses them
            )

        except Exception as e:
            logging.error(f"Unexpected error in semaphore task for page {page_num} of {doc_name}: {str(e)}")
            # Replace st.error with logging or keep if Streamlit context is intended
        ...
        # result_page_obj will always be a PageContent object (either success or error)
        logging.debug(f"Finished processing for page {page_num} of {doc_name}")
...
async def process_single_document_memory(client: genai.Client, file: dict, uploaded_files) -> List[PageContent]:
    """
    Processes a single non-PDF document from memory.
    Extracts text chunks based on type, calls Gemini for structure extraction per chunk.
    Returns a List of PageContent objects (simplified model), or a list containing
    a single error PageContent object on failure.
    """
    file_name = file.get("name", "Unknown_Document")
    file_type = file.get("type", "").lower()
    file_content = file.get("content")
    processed_pages: List[PageContent] = []

    logging.info(f"Starting page-level processing for non-PDF: '{file_name}' (type: {file_type})")

    if not file_content:
        logging.error(f"No content found for file '{file_name}'.")
        return [create_error_page(1, f"File '{file_name}' has no content.")]
    if not file_type:
        logging.error(f"Cannot determine file type for '{file_name}'.")
        return [create_error_page(1, f"Unknown file type for '{file_name}'.")]

    try:
        # --- Dispatch based on file type ---
        if file_type in ["xlsx", "xls"]:
            processed_pages = await process_tabular_data(client, file_content, file_name, "excel")
        elif file_type == "csv":
            processed_pages = await process_tabular_data(client, file_content, file_name, "csv")
        elif file_type == "docx":
            ...
            processed_pages = await process_text_document(client, file)
        else:
            logging.warning(f"Unsupported file type '{file_type}' for '{file_name}'.")
            processed_pages = [create_error_page(1, f"Unsupported file type: {file_type}")]

        if not processed_pages:
            logging.warning(f"Processing for '{file_name}' resulted in no pages.")
            processed_pages = [create_error_page(1, f"No content pages generated during processing for '{file_name}'.")]

        # --- REMOVED assembly of final dictionary ---

        logging.info(f"Finished page-level processing for non-PDF: '{file_name}'. Generated {len(processed_pages)} PageContent objects.")
        # --- Return the list of PageContent objects ---
        return processed_pages

    except Exception as e:
        logging.error(f"Core processing failed for document '{file_name}': {str(e)}", exc_info=True)
        # --- Return list with single error PageContent object ---
        return [create_error_page(1, f"Core processing error for '{file_name}': {e}")]
...
async def finalize_document(client: genai.Client, doc_name: str, pages: List[PageContent]) -> Dict:
    ...
    try:
        ...
        final_result["raw_extracted_content"]["pages"] = initial_pages_dump
         ...
         logging.error(f"[{function_name}] Failed during initial page dump for '{doc_name}': {initial_dump_err}")
         final_result["raw_extracted_content"]["pages"] = [{"page_number": i+1, "error": "Original page data could not be serialized"} for i in range(len(pages))]
    ...
    try:
        ...
        logging.debug(f"[{function_name}] Step 1: Extracting subsections from raw text for {len(pages)} pages of '{doc_name}'.")
        ...
        for page in pages:
             ...
             if isinstance(page, PageContent) and page.raw_text and not page.subsections:
                 task = extract_subsections_from_text(client, page.raw_text, page.page_number, doc_name)
                 ...
                 pages_needing_subsections.append(page) # Store reference to the page object
                  ...
                  logging.warning(f"[{function_name}] Item in pages list is not PageContent object for '{doc_name}', page {getattr(page, 'page_number', '?')}. Skipping subsection extraction for it.")
             # else: subsections already exist or no raw text, skip

        # Run tasks concurrently
        if subsection_extraction_tasks:
             logging.info(f"[{function_name}] Running {len(subsection_extraction_tasks)} subsection extraction tasks for '{doc_name}'.")
             results = await asyncio.gather(*subsection_extraction_tasks, return_exceptions=True)
             logging.info(f"[{function_name}] Finished subsection extraction tasks for '{doc_name}'.")
             ...
             if len(results) == len(pages_needing_subsections):
                 for i, result in enumerate(results):
                     ...
                     if isinstance(result, Exception):
                         logging.error(f"[{function_name}] Subsection extraction failed for page {page_to_update.page_number} of '{doc_name}': {result}")
                         ...
                         logging.error(f"[{function_name}] Unexpected result type {type(result)} for subsection extraction on page {page_to_update.page_number} of '{doc_name}'.")
             ...
             logging.info(f"[{function_name}] No pages required subsection extraction for '{doc_name}'.")


        # --- Step 2: Merge cut-off subsections ---
        logging.debug(f"[{function_name}] Step 2: Merging subsections for '{doc_name}'.")
        ...
        logging.debug(f"[{function_name}] Step 3: Extracting chapters for '{doc_name}'.")
        chapters: List[Chapter] = await extract_chapters_from_subsections(client, merged_pages)
        if not chapters:
             logging.warning(f"[{function_name}] No chapters were extracted for '{doc_name}'. Cannot proceed further.")
        ...
        logging.debug(f"[{function_name}] Step 4: Analyzing intra-chapter relationships for {len(chapters)} chapters in '{doc_name}'.")
        # This loop modifies chapter.qdrant_nodes in place
        ...
        for i, res in enumerate(intra_results):
            if isinstance(res, Exception):
                 logging.error(f"[{function_name}] Intra-chapter analysis failed for chapter {chapters[i].chapter_id} of '{doc_name}': {res}")


        # --- Step 5: Analyze relationships BETWEEN chapters & Generate Summary ---
        logging.debug(f"[{function_name}] Step 5: Analyzing inter-chapter relationships and generating summary for '{doc_name}'.")
        summary_dict = await analyze_inter_chapter_relationships(client, chapters, doc_name)


        # --- Step 6: Assemble Final Result ---
        logging.debug(f"[{function_name}] Step 6: Assembling final result for '{doc_name}'.")
        if summary_dict is None:
             logging.warning(f"[{function_name}] Failed to generate document summary details for '{doc_name}'. Using placeholder.")
             summary_dict = {
                 "title": f"Summary Failed: {doc_name}", "themes": ["error"], "questions": [],
...
async def process_tabular_data(client: genai.Client, file_content: bytes, filename: str, filetype: str) -> List[PageContent]:
    ...
    try:
        if filetype == "excel":
            ...
            try:
                 ...
                 for sheet_name, df in excel_data.items():
                     logging.info(f"Processing Excel sheet '{sheet_name}' for '{filename}' as page {page_counter}")
                     if df.empty:
                          logging.warning(f"Sheet '{sheet_name}' in '{filename}' is empty. Skipping.")
                     ...
                     page_content = await extract_structure_from_text_content(client, markdown_text, page_counter, f"{filename} (Sheet: {sheet_name})")
                     processed_pages.append(page_content)
                     page_counter += 1

            except Exception as e:
                logging.error(f"Error reading Excel file '{filename}' with pandas: {e}")
                processed_pages.append(create_error_page(1, f"Error reading Excel file '{filename}': {e}")) # Treat as single page error

        elif filetype == "csv":
            logging.info(f"Processing CSV file '{filename}' as page 1")
            try:
                ...
                # Extract structure from this markdown text
                page_content = await extract_structure_from_text_content(client, markdown_text, 1, filename)
                processed_pages.append(page_content)
            except Exception as e:
                 logging.error(f"Error reading CSV file '{filename}' with pandas: {e}")
                 processed_pages.append(create_error_page(1, f"Error reading CSV file '{filename}': {e}"))
        ...
        logging.error(f"Error processing tabular data for '{filename}': {e}")
        # Return a single PageContent object indicating the error for the whole file
        return [create_error_page(1, f"Failed to process tabular file '{filename}': {e}")]


async def process_word_document(client: genai.Client, file_content: bytes, filename: str) -> List[PageContent]:
    """Processes Word docx file by extracting text and applying structure extraction."""
    processed_pages = []
    try:
        logging.info(f"Processing Word document '{filename}'")
        doc = docx.Document(io.BytesIO(file_content))
        full_text = "\n".join([para.text for para in doc.paragraphs if para.text.strip()])

        # Option 1: Treat whole doc as one page (Simpler)
        if not full_text.strip():
             logging.warning(f"Word document '{filename}' contains no text.")
             return [create_error_page(1, "Word document is empty or contains no text.")] # Return error page

        logging.debug(f"Extracted {len(full_text)} chars from '{filename}'. Processing as single page.")
        page_content = await extract_structure_from_text_content(client, full_text, 1, filename)
        processed_pages.append(page_content)
        ...
        return [create_error_page(1, f"Failed to process Word document '{filename}': {e}")]


async def process_pptx_document(client: genai.Client, file_content: bytes, filename: str) -> List[PageContent]:
    """Processes PowerPoint pptx file by extracting text per slide and applying structure extraction."""
    processed_pages = []
    try:
        logging.info(f"Processing PowerPoint document '{filename}'")
        ppt = Presentation(io.BytesIO(file_content))

        if not ppt.slides:
             logging.warning(f"PowerPoint document '{filename}' contains no slides.")
        ...
        for i, slide in enumerate(ppt.slides, 1):
            ...
            try: # Add try-except for potential shape access errors
                for shape in slide.shapes:
                 ...
                 logging.warning(f"Error accessing title shape on slide {i} of '{filename}': {shape_error}")
            ...
            try:
                for shape in slide.shapes:
                 ...
                 logging.warning(f"Error accessing text shapes on slide {i} of '{filename}': {shape_error}")
            ...
            try:
                ...
                logging.warning(f"Error accessing notes on slide {i} of '{filename}': {notes_error}")


            slide_full_text = "\n".join(slide_texts)

            if not slide_full_text.strip():
                 logging.warning(f"Slide {i} of '{filename}' contains no text content. Skipping.")
                 # Optionally create empty page
                 # processed_pages.append(create_error_page(i, f"Slide {i} is empty."))
                 continue

            logging.debug(f"Processing slide {i} of '{filename}'")
            page_content = await extract_structure_from_text_content(client, slide_full_text, i, f"{filename} (Slide {i})")
            processed_pages.append(page_content)

        return processed_pages

    except Exception as e:
        logging.error(f"Error processing PowerPoint document '{filename}': {e}")
        return [create_error_page(1, f"Failed to process PowerPoint document '{filename}': {e}")]
...
async def process_text_document(client: genai.Client, file_data: dict) -> List[PageContent]:
    ...
    try:
        ...
        try:
            text_content = file_data["content"].decode('utf-8')
             ...
             try:
                 ...
                 logging.error(f"Failed to decode file '{filename}': {decode_err}")
                 return [create_error_page(1, f"Failed to decode file '{filename}'")]

        if not text_content.strip():
            logging.warning(f"Text document '{filename}' is empty.")
            return [create_error_page(1, f"Text document '{filename}' is empty.")]
        ...
        logging.info(f"Split '{filename}' into {len(text_chunks)} chunks for processing.")

        # --- Process each chunk ---
        for text_chunk in text_chunks:
            if not text_chunk.strip(): continue # Skip empty chunks
            page_content = await extract_structure_from_text_content(client, text_chunk, page_counter, f"{filename} (Chunk {page_counter})")
            processed_pages.append(page_content)
            page_counter += 1
        ...
        logging.error(f"Error processing text document '{filename}' type '{file_type}': {e}")
        return [create_error_page(1, f"Failed to process text document '{filename}': {e}")]
...
def display_project_ontology(ontology):
    """Display project-wide ontology (simplified, no relationships)."""
    if not ontology: return
    ontology_data = ontology.model_dump() if hasattr(ontology, 'model_dump') else ontology

    title = ontology_data.get('title', 'Project Ontology')
    overview = ontology_data.get('overview', 'No overview available.')
    document_count = ontology_data.get('document_count', 0)
    documents = ontology_data.get('documents', []) # List of titles
    global_themes = ontology_data.get('global_themes', [])
    key_concepts = ontology_data.get('key_concepts', [])
    # Entity relationships field removed
    ...
    if global_themes:
        ...
        # Use st.chip for better theme display if available, otherwise use markdown
        if hasattr(st, 'chip'):
    ...
    if documents:
        with st.expander("Documents Included In Analysis"):
            for doc_title in documents:
                st.markdown(f"- {doc_title}")

    # --- REMOVED Entity Relationship Display Section ---
