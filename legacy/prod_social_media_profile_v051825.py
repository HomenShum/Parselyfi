...
def generate_pills_html_for_card(items: List[str], placeholder: str) -> str:
    ...
    safe_items = [item for item in items if item and str(item).strip()]
    if not safe_items: 
        return f'<span class="cc-pill-placeholder">{html.escape(placeholder)}</span>'

    pills_html_list = []
    for item_name_raw in safe_items:
        # Remove html.escape here since we'll handle escaping when the items are created
        item_name = str(item_name_raw).strip()
        popout_html_content = ""
        # Check if this item has deep dive info defined (use raw name for lookup)
        if item_name in DEEP_DIVE_INFO:
            info = DEEP_DIVE_INFO[item_name]
            ...
            if info.get("inner_tags"):
                ...
                for tag_name_raw, tag_desc_raw in info["inner_tags"].items():
                    ...
                    tag_desc = str(tag_desc_raw).strip()
                    inner_tags_html += f'<span class="inner-tag" tabindex="0">{html.escape(tag_name)}'
                    inner_tags_html += f'<div class="deep-detail-popout"><p>{html.escape(tag_desc)}</p></div>'
                    inner_tags_html += '</span>'
                inner_tags_html += '</div>'

            detail_title_esc = html.escape(info.get("detail_title", item_name_raw))
            detail_snippet_esc = html.escape(info.get("detail_snippet", ""))

            # MODIFIED PART: Construct popout_html_content as a more "flat" string
            popout_html_content = (
                f'<div class="detail-popout">'
                f'<strong>{detail_title_esc}</strong>'
                f'<span class="context-snippet">{detail_snippet_esc}</span>'
                f'{inner_tags_html}'  # inner_tags_html is already valid HTML or empty
                f'</div>'
            )
            ...
            pills_html_list.append(f'<span class="cc-pill">{html.escape(item_name)}</span>')
