    ...
    if isinstance(data_node, dict):
        ...
        for index, item in enumerate(data_node):
             if isinstance(item, (dict, list)):
                 item_path_keys = current_path_keys + [f"{current_path_keys[-1]}[{index}]"] # More specific path for list items
...
if __name__ == "__main__":
    ...
    if 'edit_success_message' not in st.session_state: # For dialog success messages
    ...
    # Display success message if any, then clear it
    ...
    with st.sidebar:
            ...
            if uploaded_analysis_file.name != st.session_state.get('processed_upload_filename'):
                try:
                    ...
                    if isinstance(parsed_data, dict) and \
                       isinstance(parsed_data.get("platformSpecificAnalysis"), (dict, type(None))): # Check for core structure
                        st.session_state['analysis_data'] = parsed_data
                        # If avatar is uploaded simultaneously, use it. Otherwise, keep existing or None.
                        if uploaded_avatar_file_sidebar:
                             st.session_state['uploaded_avatar_file'] = uploaded_avatar_file_sidebar
                        # else: st.session_state['uploaded_avatar_file'] remains unchanged or None
                        ...
                        st.rerun()
                         ...
                         st.error("Invalid format: 'platformSpecificAnalysis' must be an object (dictionary) or null in the JSON.")
                         st.session_state.update({'processed_upload_filename': None, 'analysis_data': None, 'uploaded_avatar_file': None}) # Clear all on error
                    else:
                        st.error("Uploaded file does not contain a valid JSON object at the top level.")
        ...
        # Update avatar if only avatar is uploaded/changed AFTER data is loaded
             ...
             # Check if it's a new or different file
        ...
        if st.button("Load Example Data", key="load_example"):
             ...
             if isinstance(example_data, dict):
                 ...
                 st.session_state['uploaded_avatar_file'] = None # Reset avatar for example
        ...
        # Use a direct update to session_state with a callback or rely on Streamlit's auto-rerun for widgets
        show_empty_current_val = st.session_state.get('show_empty_fields', False)
        new_show_empty = st.checkbox(
            "Show Missing/Empty Fields",
            value=show_empty_current_val,
            key='show_empty_toggle_main_cb' # Unique key for checkbox
        )
        if new_show_empty != show_empty_current_val:
            st.session_state['show_empty_fields'] = new_show_empty
            st.rerun() # Explicitly rerun if the value changes

        st.divider()
        st.caption(f"Current Source: {st.session_state['current_data_source']}")

        # Edit Data interaction point (triggers dialog)
        if st.session_state.get('analysis_data'):
             with st.expander("✏️ Edit Data Sections (Dialog)", expanded=False):
                  render_edit_interaction_point() # This function now triggers the dialog
    ...
    if st.session_state.get('analysis_data'):
        ...
        coffee_card_header_cols = st.columns([0.8, 0.2]) # Adjust ratio as needed
        with coffee_card_header_cols[0]:
            st.markdown("### ☕ Your Coffee Card")
        with coffee_card_header_cols[1]:
            if data.get('yourCoffeeCard') is not None: # Only show edit if coffee card data exists
        ...
        tab_titles = ["Overview"]
        platform_keys_in_order = [] # Stores actual keys for data retrieval
        other_platforms_data_for_tab = None # Stores the list of other platform dicts

        if platform_tabs_available:
            temp_platform_analysis_obj = copy.deepcopy(platform_analysis_obj) # Work on a copy for pop
            
            other_platforms_data_raw = temp_platform_analysis_obj.pop('otherPlatforms', None)
            if isinstance(other_platforms_data_raw, list) and other_platforms_data_raw:
                ...
                if should_show_other_tab:
                    other_platforms_data_for_tab = other_platforms_data_raw
                    tab_titles.append("Other Platforms") # More descriptive tab title
                    platform_keys_in_order.append("otherPlatforms_special_key") # Special key

            sorted_platform_keys = sorted(temp_platform_analysis_obj.keys()) 
            for platform_key in sorted_platform_keys:
                platform_data = temp_platform_analysis_obj.get(platform_key)
                if isinstance(platform_data, dict): 
        ...
        if not overview_data_subset and len(tab_titles) == 1 and not platform_tabs_available:
            ...
            try:
                ...
                with tabs[0]:
                    ...
                    if overview_html:
                        ...
                        st.info("No overview data to display with current settings.")

                # Render Platform Tabs
                tab_index_offset = 1 
                for i, processed_platform_key in enumerate(platform_keys_in_order):
                    current_tab_index = i + tab_index_offset
                    if current_tab_index < len(tabs): 
                        with tabs[current_tab_index]:
                            if processed_platform_key == "otherPlatforms_special_key":
                                if other_platforms_data_for_tab:
                                    rendered_any_other = False
                                    for idx, other_platform_item_data in enumerate(other_platforms_data_for_tab):
                                         item_html = render_nested_json_html(other_platform_item_data, level=1, show_empty_fields=show_empty)
                                         if item_html: 
                                             rendered_any_other = True
                                             st.markdown(item_html, unsafe_allow_html=True)
                                             if idx < len(other_platforms_data_for_tab) - 1 and any(not is_value_empty(val) for val in other_platform_item_data.values()):
                                                 st.markdown("<hr style='border-top: 1px dashed var(--cc-accent-light-tan); margin: 1rem 0;'>", unsafe_allow_html=True)
                                    if not rendered_any_other and not show_empty: # if show_empty, some placeholder might have rendered
                                        st.info("No data displayed for 'Other Platforms' items with current settings.")
                                    elif not rendered_any_other and show_empty:
                                        st.markdown(render_missing_html() + " (All 'Other Platforms' items are empty)", unsafe_allow_html=True)

                            else: # Regular platform tab
                                 platform_data_to_render = platform_analysis_obj.get(processed_platform_key) # Use original key
                                 if platform_data_to_render: 
                                     ...
                                     if platform_html: 
                                          ...
                                          st.info(f"No data to display for {get_platform_display_name(processed_platform_key, platform_data_to_render)} with current settings.")
                                 else:
                                      st.error(f"Data inconsistency: Platform key '{processed_platform_key}' expected but data missing.")
                    else:
                        st.error(f"Tab index mismatch error for platform key: {processed_platform_key}")
            except Exception as e:
                 st.error(f"Error creating or rendering tabs: {e}. Displaying overview only as fallback.")
                 overview_html = render_nested_json_html(overview_data_subset, level=0, show_empty_fields=show_empty)
                 st.markdown(overview_html if overview_html else "<p>No overview data available.</p>", unsafe_allow_html=True)
    else:
        st.markdown("### 👋 Welcome!") 
        ...
        Use the **Display Options** in the sidebar to toggle visibility of missing fields.
        You can edit data sections using the **"Edit Data Sections (Dialog)"** expander in the sidebar, or the "✏️ Edit Card" button above the Coffee Card.
        """)