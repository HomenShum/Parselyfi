...
> 🛠️ **The animated previews below are full feature walkthroughs** — empty state → the cursor gliding to each click → the loading state → the result, with step captions and a progress bar. Each is generated by the reusable [`feature-walkthrough-gif`](.claude/skills/feature-walkthrough-gif/SKILL.md) skill (Playwright capture → Remotion render → GIF), so they show *exactly* what was clicked and what happened — not just a final-state hero shot.
